...
def login() -> bool:
    ...
    user = execute_query(query, (email, password), fetch="one")

    if user:
        if user['is_active']:
            return user  # <--- CRITICAL FIX: Returns the user dictionary (id, username), NOT True
        else:
            print("Account not verified. Please verify email.")
            return None
